...
4. **Recipe variation** — modifying an existing recipe on request ("make this vegetarian", "double the spice"), reusing the same generation mechanism as features 2–3 but scoped to a single source recipe rather than retrieval across the collection, which makes it the cheapest of the three generation-based features
