...
/// central directory, stored and deflated entries, and nothing else — the
/// encryption, spanning and Zip64 parts of the specification never appear in
/// a recipe export. Apple ships no public API for unzipping on either
/// platform, so this is the alternative to a third-party package for about a
/// hundred lines.
enum ZIPArchive {
    ...
    static func entries(in data: Data) throws -> [Entry] {
        ...
        while let signature = uint32(data, offset), signature == 0x0201_4B50 {
            ...
            if !name.hasSuffix("/"), uncompressedSize != 0xFFFF_FFFF,
               Int(uncompressedSize) <= entrySizeLimit,
               let content = content(
                   in: data,
    ...
    private static func centralDirectoryOffset(in data: Data) -> Int? {
        ...
        while offset >= lowest {
            if uint32(data, offset) == 0x0605_4B50 {
                guard let start = uint32(data, offset + 16), start != 0xFFFF_FFFF else { return nil }
