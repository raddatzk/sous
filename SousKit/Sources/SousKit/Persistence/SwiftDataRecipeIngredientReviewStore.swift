...
/// A ``RecipeIngredientReviewStore`` backed by SwiftData.
@ModelActor
public actor SwiftDataRecipeIngredientReviewStore: RecipeIngredientReviewStore {
    ...
    public func markReviewed(_ recipe: Recipe) async throws {
        ...
        if let existing = try stored(recipeID: recipe.id) {
            ...
            modelContext.insert(StoredIngredientReview(recipeID: recipe.id, reviewedContentHash: hash))
        }
    ...
    private func stored(recipeID: UUID) throws -> StoredIngredientReview? {
        var descriptor = FetchDescriptor<StoredIngredientReview>(predicate: #Predicate { $0.recipeID == recipeID })
